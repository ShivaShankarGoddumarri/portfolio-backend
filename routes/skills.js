const express = require("express");
const router = express.Router();
const Skill = require("../models/Skill");

// Get all skills
router.get("/", async (req, res) => {
  try {
    const skills = await Skill.find();
    res.json(skills);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});

// Seed skills (for initial setup)
router.post("/seed", async (req, res) => {
  try {
    const skillsData = [
      {
        title: "Web Development",
        skills: [
          {
            name: "React Js",
            image:
              "data:image/svg+xml;base64,",
          },
          {
            name: "JavaScript",
            image:
              "https://upload.wikimedia.org/wikipedia/commons/thumb/6/6a/JavaScript-logo.png/800px-JavaScript-logo.png",
          },
          {
            name: "TypeScript",
            image:
              "https://th.bing.com/th?id=OIP.kUorSTg4WAJvluB7R0BUegHaHa&w=250&h=250&c=8&rs=1&qlt=90&o=6&dpr=1.3&pid=3.1&rm=2",
          },
          {
            name: "Sharepoint",
            image:
              "https://th.bing.com/th?id=OIP.g7uX5AXrFcBQ1cAiCg24JQHaHa&w=250&h=250&c=8&rs=1&qlt=90&o=6&dpr=1.3&pid=3.1&rm=2",
          },
          {
            name: "HTML",
            image: "https://www.w3.org/html/logo/badge/html5-badge-h-solo.png",
          },
          {
            name: "CSS",
            image:
              "https://upload.wikimedia.org/wikipedia/commons/thumb/d/d5/CSS3_logo_and_wordmark.svg/1452px-CSS3_logo_and_wordmark.svg.png",
          },
          {
            name: "SCSS",
            image:
              "https://th.bing.com/th?q=Scss+Icon.png&w=120&h=120&c=1&rs=1&qlt=90&cb=1&dpr=1.3&pid=InlineBlock&mkt=en-IN&cc=IN&setlang=en&adlt=moderate&t=1&mw=247",
          },
          {
            name: "Material UI",
            image:
              "data:image/png;base64,",
          },
          {
            name: "Redux",
            image:
              "https://th.bing.com/th/id/R.8251fe76d7487a28b07603515b300964?rik=H7lrBp652zDaWg&riu=http%3a%2f%2fassets.stickpng.com%2fimages%2f5848309bcef1014c0b5e4a9a.png&ehk=N%2fi7UuvK4YyoLcxusdDEWeftPaGIfNJl2VmpTOtLt%2bA%3d&risl=&pid=ImgRaw&r=0",
          },
          {
            name: "Jest",
            image:
              "https://www.danielcornock.co.uk/assets/images/article-icons/jest.png",
          },
          {
            name: "VS Code",
            image:
              "https://upload.wikimedia.org/wikipedia/commons/thumb/9/9a/Visual_Studio_Code_1.35_icon.svg/512px-Visual_Studio_Code_1.35_icon.svg.png?20210804221519",
          },
          {
            name: "Git",
            image:
              "https://tse1.mm.bing.net/th?id=OIP.CSOa8Mk8jL35E1_02q2spAAAAA&pid=Api&P=0&h=180",
          },
          {
            name: "GitHub",
            image:
              "https://github.githubassets.com/images/modules/logos_page/GitHub-Mark.png",
          },
        ],
      },
      {
        title: "Core Electronics",
        skills: [
          {
            name: "Product Design",
            image: "https://cdn-icons-png.flaticon.com/128/3852/3852813.png",
          },
          {
            name: "Embedded Systems",
            image: "https://cdn-icons-png.flaticon.com/128/5721/5721444.png",
          },
          {
            name: "Circuit Designing",
            image: "https://cdn-icons-png.flaticon.com/128/4645/4645248.png",
          },
        ],
      },
      {
        title: "Microsoft Office",
        skills: [
          {
            name: "Word",
            image:
              "https://logodownload.org/wp-content/uploads/2018/10/word-logo-8.png",
          },
          {
            name: "Excel",
            image:
              "https://logodownload.org/wp-content/uploads/2020/04/excel-logo-0.png",
          },
          {
            name: "Power Point Presentation",
            image:
              "https://logodownload.org/wp-content/uploads/2020/04/microsoft-powerpoint-logo-0.png",
          },
        ],
      },
      {
        title: "Creative Design",
        skills: [
          {
            name: "Canva",
            image:
              "https://github.com/ShivaShankarGoddumarri/User-Images/assets/96565316/846dceab-7768-4811-b13f-c27456e5323e",
          },
          {
            name: "Figma",
            image:
              "https://cdn.icon-icons.com/icons2/2699/PNG/512/figma_logo_icon_170157.png",
          },
          {
            name: "Picsart",
            image:
              "https://play-lh.googleusercontent.com/hexunszQESgaMxZdfuGvYdNnS6fRebRqW5lECqadoqlTL3fbJbLL6NKceHlQ58ALXcE",
          },
          {
            name: "Kinemaster",
            image:
              "https://cdn141.picsart.com/009b40e9-5c90-4dcc-8171-6ff04d177c95/392124013016211.png",
          },
        ],
      },
    ];
    await Skill.insertMany(skillsData);
    res.json({ message: "Skills seeded!" });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});

module.exports = router;
